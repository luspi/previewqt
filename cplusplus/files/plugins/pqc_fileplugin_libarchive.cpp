...
const QSize PQCFilePluginLibarchive::loadSize(QString path) {
    ...
    while(archive_read_next_header(a, &entry) == ARCHIVE_OK) {
        ...
        if(filenameinside == compressedFilename || (compressedFilename.isEmpty() && !QFileInfo(filenameinside).suffix().isEmpty())) {
            ...
            QSize origSize = PQCFileHandler::get().getSize(tempFile.fileName());
...
const QImage PQCFilePluginLibarchive::loadImage(QString path, QSize requestedSize, QSize &origSize, QString &error) {
    ...
    if(suffix == "cbr" || suffix == "rar") {
        ...
        if(dir.mkpath(tmpDir)) {
            ...
            if(p.waitForStarted()) {
                ...
                QImage img = PQCFileHandler::get().getImage(tmpDir + compressedFilename, QSize(-1,-1), origSize, error);
                QDir dir(tmpDir);
    ...
    while(archive_read_next_header(a, &entry) == ARCHIVE_OK) {
        ...
        if(filenameinside == compressedFilename || (compressedFilename.isEmpty() && !QFileInfo(filenameinside).suffix().isEmpty())) {
            ...
            img = PQCFileHandler::get().getImage(tempFile.fileName(), QSize(-1,-1), origSize, error);
            if(img.isNull())
    ...
    // Scale image if necessary
    if(!requestedSize.isEmpty()) {
        img = img.scaled(origSize.scaled(requestedSize, Qt::KeepAspectRatio), Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
    }
