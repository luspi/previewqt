...
const QSize PQCFilePluginPDF::loadSize(QString path) {
    ...
    return (p->pageSizeF()*(100.0/72.0)).toSize();
    ...
    QSizeF _pageSize = (doc.pagePointSize(page)/72.0*qApp->primaryScreen()->physicalDotsPerInch())*(100.0/72.0);
...
const QImage PQCFilePluginPDF::loadImage(QString path, QSize requestedSize, QSize &origSize, QString &error) {
    ...
    QSizeF _pageSize = doc.pagePointSize(page)*(100.0/72.0);
