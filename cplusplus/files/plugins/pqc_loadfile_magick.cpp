...
PQCLoadFileMagick::PQCLoadFileMagick() {}
...
QString PQCLoadFileMagick::load(QString filename, QSize maxSize, QSize &origSize, QImage &img) {
    ...
    if(suf != "") {
        ...
        if(PQCFileFormats::get().getMagick().keys().contains(suf.toLower()))
            mgs = PQCFileFormats::get().getMagick().value(suf.toLower()).toStringList();
    }
    if(mimetype != "") {
        if(PQCFileFormats::get().getMagickMimeType().keys().contains(mimetype)) {
            const QStringList lst = PQCFileFormats::get().getMagickMimeType().value(mimetype).toStringList();
