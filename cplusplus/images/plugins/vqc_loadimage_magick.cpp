...
VQCLoadImageMagick::VQCLoadImageMagick() {}

QString VQCLoadImageMagick::load(QString filename, QSize maxSize, QSize &origSize, QImage &img) {
    ...
    if(suf != "") {
        ...
        if(VQCImageFormats::get().getMagick().keys().contains(suf.toLower()))
            mgs = VQCImageFormats::get().getMagick().value(suf.toLower()).toStringList();
    }
    if(mimetype != "") {
        if(VQCImageFormats::get().getMagickMimeType().keys().contains(mimetype)) {
            const QStringList lst = VQCImageFormats::get().getMagickMimeType().value(mimetype).toStringList();
