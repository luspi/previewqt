...
VQCProviderFull::VQCProviderFull() : QQuickImageProvider(QQuickImageProvider::Image) {}

VQCProviderFull::~VQCProviderFull() {}

QImage VQCProviderFull::requestImage(const QString &url, QSize *origSize, const QSize &requestedSize) {

    qDebug() << "args: url =" << url;
    qDebug() << "args: requestedSize =" << requestedSize;

    QString filename = VQCScripts::cleanPath(QByteArray::fromPercentEncoding(url.toUtf8()));
    ...
    // Load image
    QImage ret;
    VQCLoadImage::get().load(filename, requestedSize, *origSize, ret);
