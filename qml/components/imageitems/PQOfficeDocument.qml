...
Item {
    ...
    Image {
        ...
        Component.onCompleted: {
            ...
            if(PQCConstants.currentSource.includes("::DOC::")) {
                doc_top.currentPage = PQCConstants.currentSource.split("::DOC::")[0]*1
                source = "image://full/" + PQCScriptsFilesPaths.toPercentEncoding(PQCConstants.currentSource)
            } else {
                doc_top.currentPage = 0
                source = "image://full/" + PQCScriptsFilesPaths.toPercentEncoding("0::DOC::%2".arg(PQCConstants.currentSource))
            }
        }
    ...
    property int pageCount: PQCFileHandler.getNumPages("libreoffice", PQCConstants.currentSource)
    ...
    onCurrentPageChanged: {
        ...
        if(PQCConstants.currentSource.includes("::DOC::")) {
            imageitem.source = "image://full/" + PQCScriptsFilesPaths.toPercentEncoding("%1::DOC::%2".arg(currentPage).arg(PQCConstants.currentSource.split("::DOC::")[1]))
        } else {
            imageitem.source = "image://full/" + PQCScriptsFilesPaths.toPercentEncoding("%1::DOC::%2".arg(currentPage).arg(PQCConstants.currentSource))
        }
